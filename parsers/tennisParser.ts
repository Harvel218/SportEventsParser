...
export const tennisParser = (event: EventInterface): ParsedEventInterface => {
	if (typeof event.score !== 'string') {
		throw new Error(
			`tennisParser: Wrong score has been provided, expected string but received: ${event.score}`
		);
	}
